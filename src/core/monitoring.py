...
async def get_version() -> VersionResponse:
    ...
    try:
        # Resolves only when installed as a distribution; the container image
        # runs from source (uv sync --no-install-project), so fall back cleanly.
        app_version = version(settings.app_name)
